...
class TituladosImport implements ToCollection, WithHeadingRow, SkipsOnFailure, WithChunkReading, WithBatchInserts
{
        ...
        foreach ($rows as $index => $row) {
            if (isset($row['apellido_paterno']) && isset($row['apellido_materno']) && isset($row['nombres'])) {
                $alumno = Alumno::where('paterno', trim($row['apellido_paterno']))
                    ...
                    ->first();
                if ($alumno) {
                    $escuela = Escuela::where('nombre', $row['carrera'])->first();
                    $condicion_alumno = CondicionAlumno::where('alumno_id', $alumno->id)
                        ->where('condicion_id', 3)->first();
